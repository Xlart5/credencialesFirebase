...
class _NuevoContratoSheetState extends State<NuevoContratoSheet> {
  ...
  Future<void> _generarNuevoContrato() async {
    ...
    // Ejecuta el POST con todos los datos y el nuevo cargo
    ...
    if (mounted) {
      ...
      if (success) {
        ...
        Navigator.pop(context); // Cierra el modal de historial que estaba abajo
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("¡Nuevo Contrato Registrado con Éxito!"),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Error al registrar nuevo contrato"),
            backgroundColor: Colors.red,
          ),
        );
      }
