...
class _CertificadoSheetState extends State<CertificadoSheet> {
  ...
  void initState() {
    super.initState();
    // Si ya terminó, buscamos sus fechas en el historial
    if (_estaTerminado) {
      WidgetsBinding.instance.addPostFrameCallback(
        (_) => _cargarUltimoHistorial(),
      );
    }
  }
  ...
  Future<void> _seleccionarFecha(BuildContext context, bool esInicio) async {
    if (_estaTerminado) return; // Si ya terminó, no dejamos editar fechas
    ...
    bool confirmar =
        await showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text(
              "Se guardarán las fechas y se cerrará el contrato activo de esta persona.",
            ),
            actions: [
              TextButton(
    ...
    try {
      ...
      if (mounted) {
        ...
        if (success) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Contrato terminado exitosamente"),
  ...
  // ===========================================
  // 2. IMPRIMIR CERTIFICADO (AQUÍ ESTÁ LA CORRECCIÓN DEL ERROR)
  // ===========================================
  Future<void> _imprimirCertificado() async {
    ...
    // 🔥 LA SOLUCIÓN: Empaquetamos a la persona con sus fechas
    ...
    // Ahora sí enviamos la lista con el paquete
    ...
    await Printing.layoutPdf(
      ...
      name: 'Certificado_${widget.employee.ci}.pdf',
    );
  }

  // ===========================================
  // 3. INICIAR NUEVO CONTRATO
  // ===========================================
  Future<void> _iniciarNuevoContrato() async {
    bool confirmar =
        await showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text(
              "El estado cambiará a 'PERSONAL REGISTRADO'. Podrás asignarle un nuevo cargo desde el Dashboard principal.",
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(ctx, false),
    ...
    if (mounted) {
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Persona lista para nuevo contrato"),
            backgroundColor: Colors.green,
          ),
        );
      }
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton.icon(
                                ...
                                label: Text(
                                  _isProcessing
                                      ? "Guardando..."
                                      : "Guardar Fechas y Cerrar Contrato",
                                  style: const TextStyle(
                                    fontWeight: FontWeight.bold,
                                  ),
                                ),
                          ...
                          if (_estaTerminado) ...[
                            SizedBox(
                              ...
                              child: ElevatedButton.icon(
                                ...
                                onPressed: _fechaIngreso == null
                                    ? null
                                    : _imprimirCertificado,
                                icon: const Icon(Icons.print),
                                label: const Text(
                                  "1. Imprimir Certificado PDF",
            ...
            child: Row(
              ...
              children: [
                ...
                Icon(
                  ...
                  size: 18,
                  color: bloqueado ? Colors.grey : AppColors.primaryDark,
                ),
              ],
            ),
          ),
